\section{Physics Interpolation: Continuous Split Velocity Fields}

\subsection{Architectural Role}

The interpolation module is a downstream consumer of the frozen CFD Version 1
library. It does not generate geometry, regenerate meshes, solve Stokes systems,
or alter CFD outputs. The scientific workflow is:

\[
\mathrm{Hydraulics}\rightarrow \alpha(t),
\qquad
\mathrm{CFD~Version~1}\rightarrow \mathbf{u}(x,y;\alpha_i),
\qquad
...
height-averaged background-flow approximation for the junction patch. Future ML
modules can sample this continuous background field, but no transformer
...
library = VelocityFieldLibrary.from_directory(
    "outputs/physics/junction_cfd/solutions"
)
